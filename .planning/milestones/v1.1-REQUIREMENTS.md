...
- [x] **ENH-01**: Upstream gluco-hub-rs supports multi-patient polling from a single LibreLink Up login — v1.1
- [x] **ENH-02**: Per-patient MQTT sensor entities with distinct unique_id, topic_prefix, and client_id — v1.1
- [x] **ENH-03**: HA config schema updated for per-patient options (list or auto-discovery) — v1.1
- [x] **ENH-04**: Per-patient token isolation — separate token cache per patient — v1.1

### API Resilience (ENHN-04)

- [x] **ENH-05**: Upstream gluco-hub-rs handles HTTP 429 rate-limit with Retry-After backoff — v1.1
- [x] **ENH-06**: API version header (llu_version) exposed in add-on config — v1.1
- [x] **ENH-07**: JSON schema hardening — critical fields made Option with graceful defaulting — v1.1
- [x] **ENH-08**: Schema fingerprint logging for detecting LibreLink Up API changes — v1.1

### Cross-cutting

- [x] **ENH-09**: Backward compatibility — existing single-patient configs keep working — v1.1
- [x] **ENH-10**: MQTT discovery collision prevention — guaranteed unique unique_id per patient — v1.1
